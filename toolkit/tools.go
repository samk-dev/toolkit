...
// Tools is the type used to instantiate this module. Any variables of this type will have
...
func (t *Tools) UploadFiles(r *http.Request, uploadDir string, rename ...bool) ([]*UploadedFile, error) {
	...
	if err != nil {
		return nil, errors.New("the uploaded file is huge")
	}
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, header := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				if renameFile {
					uploadedFile.NewFileName = fmt.Sprintf("%s%s", t.RandomStringGenerator(25), filepath.Ext(header.Filename))
				} else {
				...
				// TODO: Sanitize file metadata && return it the rest.
